...
// ── App ────────────────────────────────────────────────────────────────────
export default function MarketLens({ __OVERRIDE_INSIGHTS }) {
  const [tab, setTab] = useState("all");

  const INSIGHTS = Array.isArray(__OVERRIDE_INSIGHTS) && __OVERRIDE_INSIGHTS.length
    ? __OVERRIDE_INSIGHTS
    : MOCK_INSIGHTS;

  const featured = INSIGHTS.filter((i) => i.conviction >= 80);
  const feed =
    tab === "bullish"
      ? INSIGHTS.filter((i) => i.direction === "bullish")
      : tab === "bearish"
        ? INSIGHTS.filter((i) => i.direction === "bearish")
        : INSIGHTS;
