  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', padding: '24px', fontFamily: FONT, color: '#e2e8f0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 9, letterSpacing: 3, color: '#334155', textTransform: 'uppercase', marginBottom: 4 }}>The Brain</div>
          <div style={{ fontSize: 20, fontWeight: 'bold', color: '#e2e8f0', letterSpacing: 1 }}>Scoreboard</div>
        </div>
        <div style={{ fontSize: 10, color: '#334155', letterSpacing: 1 }}>{time}</div>
      </div>
      ...
      {positions.length > 0 && (
        <div style={{ marginBottom: 28, border: `1px solid rgba(251,191,36,0.2)`, borderRadius: 8, overflow: 'hidden', background: 'rgba(0,10,30,0.6)' }}>
          <div style={{ padding: '12px 20px', borderBottom: `1px solid rgba(251,191,36,0.12)`, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: 9, color: '#334155' }}>{positions.length} active</span>
          </div>
              ...
              <thead>
                <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                  {['Symbol', 'Qty', 'Avg Entry', 'Market Value', 'Unrealized P&L', 'Change%'].map((h, i) => (
                    <th key={h} style={{ padding: '8px 16px', textAlign: i === 0 ? 'left' : 'right', fontSize: 9, letterSpacing: 1.5, color: '#334155', textTransform: 'uppercase', fontWeight: 'normal' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
      ...
      <div style={{ marginBottom: 28, border: `1px solid ${BORDER}`, borderRadius: 8, overflow: 'hidden', background: 'rgba(0,10,30,0.6)' }}>
        <div style={{ padding: '12px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ marginLeft: 'auto', fontSize: 9, color: '#334155' }}>Alpaca paper + Kalshi demo</span>
        </div>
        {trades.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: 13, color: '#334155' }}>No trades yet</div>
            <div style={{ fontSize: 10, color: '#1e293b', marginTop: 6 }}>Scanner to Risk to Execution pipeline running</div>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                  {['Date', 'Symbol', 'Desk', 'Side', 'Qty', 'Entry', 'Exit', 'P&L', 'Status'].map((h, i) => (
                    <th key={h} style={{ padding: '8px 14px', textAlign: i >= 4 ? 'right' : 'left', fontSize: 9, letterSpacing: 1.5, color: '#334155', textTransform: 'uppercase', fontWeight: 'normal' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {trades.map((t: any, i: number) => {
                  ...
                  const pnlColor = pnl == null ? '#475569' : pnl > 0 ? '#00ff9f' : pnl < 0 ? '#ff3c6e' : '#64748b'
                  const sideColor = String(t.side) === 'buy' ? '#60a5fa' : '#fb923c'
                  const statusColor = t.status === 'open' ? '#fbbf24' : (t.status === 'filled' || t.status === 'closed') ? '#00ff9f' : '#475569'
                  return (
                    <tr key={t.id ?? i} style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                      <td style={{ padding: '10px 14px', color: '#475569', fontSize: 10 }}>{fmtDate(t.opened_at)}</td>
                      <td style={{ padding: '10px 14px', color: '#fff', fontWeight: 'bold' }}>{t.symbol ?? '—'}</td>
                      <td style={{ padding: '10px 14px', color: '#475569', textTransform: 'capitalize' }}>{t.desk ?? '—'}</td>
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                {['Date', 'Status', 'Trades', 'P&L'].map((h, i) => (
                  <th key={h} style={{ padding: '8px 14px', textAlign: i >= 2 ? 'right' : 'left', fontSize: 9, letterSpacing: 1.5, color: '#334155', textTransform: 'uppercase', fontWeight: 'normal' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(outcomes ?? []).length === 0 && (
                <tr><td colSpan={4} style={{ padding: '32px 14px', textAlign: 'center', color: '#334155', fontSize: 10 }}>No strategy outcomes yet</td></tr>
              )}
              {(outcomes ?? []).slice(0, 20).map((o: any) => {
                ...
                return (
                  <tr key={o.id} style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                    <td style={{ padding: '10px 14px', color: '#475569', fontSize: 10 }}>{fmtDate(String(o.created_at))}</td>
